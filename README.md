...
일반 공격·자동 방어, 테스트 덱 시장과 턴당 1장 구매를 지원합니다. 스킬 지급·공격·선택 방어·훔쳐보기·공개를 지원합니다. 게임 화면, 가축 유지·부족장·승리 판정을 포함한 전체 게임 엔진은 후속 개발 단계입니다.
...
가축의 카드 구매 대체는 구현했습니다. 가축 유지, 부족장, 실제 네트워크 전송은 아직 구현하지 않았습니다.
...
- 현재 흐름은 `AWAITING_ACQUISITION → AFTER_ACQUISITION → ENDED`입니다. 첫 단계에서 일반 공격을 최대 1회 할 수 있고, 자원 획득을 선택하면 공격 단계가 닫힙니다. 획득 후 최대 1장 구매가 가능합니다. 스킬은 본인 턴 중 별도로 사용하며 방어 응답 중에는 행동이 잠깁니다. 유지·부족장·승리 판정은 아직 수행하지 않습니다.
...
- 실행 예제: `npm run demo:combat`. 부족장 점수는 아직 구현하지 않았습니다.
...
- 3점 카드는 스킬 1장을 즉시 지급합니다. 덱 소진 시에만 `pendingSkillRewards`로 미지급 보상을 남깁니다.
...
- `attackSkill(playerId, defenderId, target)`: 불 없이 기본 자원/가축 토큰 1개 공격. 여러 장 사용 가능.
- `peekHidden(playerId, targetId)`: 사용 당시의 은닉 구성을 반환. **시전자에게만 전달**하며 공개 방송하면 안 됩니다.
- `revealHidden(playerId, targetId, faceDownSlot)`: 0부터 은닉 개수-1까지의 뒷면 자리 선택. 선택된 토큰 1개를 모두에게 공개.
- `defenseOptions(playerId)`: 응답할 방어자만 조회하는 비공개 선택 정보.
- `respondDefense(playerId, turnNumber, actionId, useSkill)`: 방어자만 현재 공격에 응답. 중복/지난 턴 응답은 거부.
- `publicState().currentTurn.pendingDefense`가 있으면 응답 전 다른 행동을 할 수 없습니다.
- 방어 카드 보유 여부를 감추기 위해 스킬을 한 장이라도 가진 피격자는 동일한 응답 절차를 거칩니다. 방어 카드가 없으면 피해 수락만 가능합니다.
- `privatePlayerState(id).skills`와 `peeks`는 해당 플레이어 전용입니다. 훔쳐보기 기록은 과거 시점의 스냅샷입니다.
- 서버용 엔진 API입니다. 실제 화면/토큰 선택 UI와 인증된 사용자별 전송 계층은 아직 없습니다.
- 베타 스킬 덱은 4종 각 5장으로 무작위 셔플합니다. 테스트에서는 `{ skillDeck: ["ATTACK", "PEEK"] }`처럼 순서를 주입할 수 있습니다. 덱 순서나 난수는 공개 상태에 포함하지 않습니다.
- 사용한 카드는 재순환하지 않습니다. 기본 덱은 점수 카드의 최대 스킬 보상 수보다 크므로 일반 베타 플레이에서 소진되지 않습니다.
- 일반 공격을 먼저 하거나 생략한 뒤 스킬을 사용합니다. 스킬을 먼저 사용하면 일반 공격은 생략한 것으로 처리합니다.
