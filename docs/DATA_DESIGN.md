...
- 일반 공격/자동 방어는 획득 전에, 카드 구매는 획득 후 최대 1회로 연결되었다. 스킬 사용, 가축 유지, 부족장, 승리 판정은 아직 연결되지 않았다.
...
- `spendTokens`는 일반 공격 비용·피해 및 카드 구매 로직이 사용하는 공통 차감 API다. 행동의 적법성은 각 행동 시스템에서 검증한다.
...
## 19. 점수 카드·시장·구매
- `PointCard`: ID, 등급, 인쇄 비용, 점수, 3점 스킬 보상 여부를 불변 스냅샷으로 제공한다. 등급별 비용 패턴을 생성 시 검증한다.
- `Market`: 플레이어 수별 등급 슬롯과 비공개 공급 덱을 관리한다. 공개 상태는 고정 슬롯 목록이며 소진된 슬롯은 null이다.
- `PurchasePayment`: 가축으로 대체할 비용을 받아 실제 기본 자원 지불량과 가축 0/1개를 계산한다.
- `LivestockPolicy`: 기본 `BETA_SPLIT`, 비교용 `SAME_RESOURCE`. TurnManager 생성 시 고정하며 도중에 바꾸지 않는다.
- `CardPurchase`: 공개 카드 조회 → 지불 계획 검증 → Player 결제/보상 반영 → 같은 등급 보충. 동기 처리 사이에 외부 콜백을 실행하지 않는다.
- `Player.purchasePointCard`: 인쇄 비용과 실제 지불의 합치, 최대 1가축, 중복 소유, 점수 오버플로를 검증한 후 공개분 우선 차감과 소유·점수를 함께 갱신한다.
- `purchasedCards`는 구매한 카드의 공개 스냅샷 목록이다. `pendingSkillRewards`는 3점 카드 구매로 적립한 스킬 수령 대기 장수다. 실제 스킬 카드는 아직 배정하지 않는다.
- `Turn.purchase`: 현재 플레이어의 획득 후 상태에서만 최대 1회 성공을 허용한다. 실패하면 횟수를 소비하지 않는다.
- `TurnManager.purchase(playerId, cardId, replacement)`: 예를 들어 `{ WOOD: 1, FOOD: 1 }`은 가축 1개로 해당 비용 2개를 대체한다. 가축을 사용하지 않으면 생략한다.
- 공개 구매 결과는 플레이어와 카드 정보다. 숨긴 자원의 잔량이나 실제 지불 자원 목록을 별도 방송하지 않는다. 공개 재고/가축/총량 변경에 따른 추론은 가능하다.
- 초기 PlayerSetup은 시작 자원/점수 설정용이며 구매 이력까지 복구하는 완전한 저장/로드 API는 아직 아니다.

비교 분석: [가축 베타 분석](LIVESTOCK_BETA_ANALYSIS.md). 이 분석의 균등 자원 상태 비율을 실제 승률로 해석하지 않는다.
