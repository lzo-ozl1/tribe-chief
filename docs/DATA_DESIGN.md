...
현재 구현은 20절의 종류별 수량 모델을 사용한다. 동일 종류 카드는 효과가 같아 개별 카드 ID를 부여하지 않는다.
...
- 일반 공격/자동 방어는 획득 전에, 카드 구매는 획득 후 최대 1회로 연결되었다. 스킬은 본인 턴 중 연결되었다. 가축 유지, 부족장, 승리 판정은 아직 연결되지 않았다.
...
- `BasicAttackResult`: 공격자 ID, 방어자 ID, 지정 자원, `HIT / DEFENDED / VOID / PENDING_DEFENSE` 결과를 포함한다. 방어 스킬 사용 시 `defenseSkillUsed: true`, `leaderScoreEligible: false`를 추가한다.
...
- 일반 공격 결과를 후속 부족장 시스템이 사용할 수 있지만 이번 단계에서는 점수를 자동 지급하지 않는다. 스킬이 개입한 결과에는 점수 제외 정보를 기록한다.
...
- `purchasedCards`는 구매한 카드의 공개 스냅샷 목록이다. `pendingSkillRewards`는 3점 카드 구매로 적립한 스킬 수령 대기 장수다. TurnManager는 구매 직후 실제 카드로 지급하고 대기 수량을 줄인다. 공급 덱 소진 시 대기 수량을 유지한다.
...
## 20. 스킬 지급·선택 방어·정보 효과
- `SkillInventory`: ATTACK/DEFENSE/PEEK/REVEAL별 비공개 장수. 잘못된 종류/수량/초과 차감을 원자적으로 거부한다.
- `SkillDeck`: 생성 시 입력을 검증·복사하며 공개 뷰에 남은 장수만 제공한다. 실제 공급 순서는 엔진만 소유한다.
- `TurnManager.purchase`: 스킬 보유량 오버플로를 결제 전에 확인한다. 구매 성공 후에만 보상을 지급하고 덱을 인출한다.
- `PublicPlayerState.skillCardCount`는 합계만 포함한다. `PrivatePlayerState.skills`와 `peeks`는 소유자 전용이다.
- `SkillActionResult`는 행동 ID/시전자/대상/종류/공개 결과를 담는다. 훔쳐본 재고는 포함하지 않는다.
- `PrivatePeek`는 사용 당시 은닉 재고의 불변 사본이다. TurnManager는 소유자 기록에 턴 번호도 보관한다.
- `pendingDefense`는 현재 공격을 식별한다. 실제 방어 카드 유무는 공개하지 않고 `defenseOptions`로 방어자에게만 알려준다.
- 스킬 보유 장수가 1 이상인 경우 종류에 관계없이 응답을 기다려 대기 여부로 방어 카드 유무가 드러나는 것을 막는다.
- 응답은 플레이어 ID·턴 번호·행동 ID·boolean 선택을 검증한다. 대기 중 획득/구매/스킬/일반 공격/종료를 막고 응답 후 원래 단계에서 계속한다.
- 공격 스킬은 선언 시 소모한다. 방어 스킬은 사용을 선택했을 때만 소모한다. 정보 스킬은 유효한 요청에만 소모한다.
- 공개 스킬은 숨긴 토큰 총수에 대응하는 가상 자리 중 시전자가 선택한 자리를 처리한다. 비공개 무작위 오프셋으로 종류를 결정하여 자원 종류가 아니라 토큰 한 개 기준으로 추출한다. 실제 화면은 이 API 위에 구현할 수 있다.
- 현재 API는 신뢰된 엔진 호출용이다. 네트워크 계층에서 인증된 호출자 ID와 각 비공개 응답의 수신자를 검증해야 한다.
